...
def test_free_space_propagation():
    ...
    os.makedirs("./outputs/free_space", exist_ok=True)
    ...
    Image.fromarray((intensity_img * 255).astype(np.uint8)).save(
        "./outputs/free_space/point_source_diffraction.png"
    )
    ...
    Image.fromarray((slit_img * 255).astype(np.uint8)).save(
        "./outputs/free_space/slit_diffraction.png"
    )
    ...
    for dist in distances:
        ...
        Image.fromarray((z_img * 255).astype(np.uint8)).save(
            f"./outputs/free_space/diffraction_z{int(dist*100):03d}cm.png"
        )
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("./outputs/free_space/gaussian_propagation.png", dpi=150)
        plt.close()
    ...
    print("Results saved to ./outputs/free_space/")
    print("=" * 60)
